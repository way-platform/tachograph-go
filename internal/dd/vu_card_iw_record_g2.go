...
//   - Bytes 72-90: fullCardNumberAndGeneration (FullCardNumberAndGeneration)
...
func (opts UnmarshalOptions) UnmarshalVuCardIWRecordG2(data []byte) (*ddv1.VuCardIWRecordG2, error) {
	...
	if len(data) < lenVuCardIWRecordG2 {
		return nil, fmt.Errorf("invalid data length for VuCardIWRecordG2: got %d, want at least %d", len(data), lenVuCardIWRecordG2)
	}
	...
	// manualInputFlag (1 byte) — may be absent in older devices; default to false.
	if len(data) > idxManualInputFlag {
		record.SetManualInputFlag(data[idxManualInputFlag] != 0)
	}
