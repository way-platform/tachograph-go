...
func unmarshalFaultRecord(data []byte, rec *cardv1.FaultsData_Record) error {
	...
	if enumNum, found := dd.SetEnumFromProtocolValue(enumDesc, int32(faultType)); found {
	...
	rec.SetFaultBeginTime(dd.ReadTimeReal(bytes.NewReader(data[offset : offset+4])))
	offset += 4
	...
	rec.SetFaultEndTime(dd.ReadTimeReal(bytes.NewReader(data[offset : offset+4])))
	offset += 4
