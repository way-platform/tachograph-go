...
- **Special Types**:
  - `StringValue`: For code-paged strings (includes `encoding` field, which
    is `required` — **never** use for IA5 strings or you'll fail protovalidate).
  - `Ia5StringValue`: For `IA5String` fields (pure ASCII, no encoding prefix)
    — use whenever the ASN.1 definition says `IA5String`.
