// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.6
// 	protoc        (unknown)
// source: wayplatform/connect/tachograph/datadictionary/v1/control_type.proto

package datadictionaryv1

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

// Represents the activities carried out during a control.
//
// See Data Dictionary, Section 2.53.
//
// ASN.1 Definition:
//
//	ControlType ::= OCTET STRING (SIZE(1))
//
// The byte is a bitmask with the following structure: 'cvpdexxx'B
// - 'c': card downloading
// - 'v': VU downloading
// - 'p': printing
// - 'd': display
// - 'e': roadside calibration checking (Gen2+)
// - 'xxx': RFU
type ControlType struct {
	state                          protoimpl.MessageState `protogen:"opaque.v1"`
	xxx_hidden_CardDownloading     bool                   `protobuf:"varint,1,opt,name=card_downloading,json=cardDownloading,proto3"`
	xxx_hidden_VuDownloading       bool                   `protobuf:"varint,2,opt,name=vu_downloading,json=vuDownloading,proto3"`
	xxx_hidden_Printing            bool                   `protobuf:"varint,3,opt,name=printing,proto3"`
	xxx_hidden_Display             bool                   `protobuf:"varint,4,opt,name=display,proto3"`
	xxx_hidden_CalibrationChecking bool                   `protobuf:"varint,5,opt,name=calibration_checking,json=calibrationChecking,proto3"`
	unknownFields                  protoimpl.UnknownFields
	sizeCache                      protoimpl.SizeCache
}

func (x *ControlType) Reset() {
	*x = ControlType{}
	mi := &file_wayplatform_connect_tachograph_datadictionary_v1_control_type_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ControlType) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ControlType) ProtoMessage() {}

func (x *ControlType) ProtoReflect() protoreflect.Message {
	mi := &file_wayplatform_connect_tachograph_datadictionary_v1_control_type_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *ControlType) GetCardDownloading() bool {
	if x != nil {
		return x.xxx_hidden_CardDownloading
	}
	return false
}

func (x *ControlType) GetVuDownloading() bool {
	if x != nil {
		return x.xxx_hidden_VuDownloading
	}
	return false
}

func (x *ControlType) GetPrinting() bool {
	if x != nil {
		return x.xxx_hidden_Printing
	}
	return false
}

func (x *ControlType) GetDisplay() bool {
	if x != nil {
		return x.xxx_hidden_Display
	}
	return false
}

func (x *ControlType) GetCalibrationChecking() bool {
	if x != nil {
		return x.xxx_hidden_CalibrationChecking
	}
	return false
}

func (x *ControlType) SetCardDownloading(v bool) {
	x.xxx_hidden_CardDownloading = v
}

func (x *ControlType) SetVuDownloading(v bool) {
	x.xxx_hidden_VuDownloading = v
}

func (x *ControlType) SetPrinting(v bool) {
	x.xxx_hidden_Printing = v
}

func (x *ControlType) SetDisplay(v bool) {
	x.xxx_hidden_Display = v
}

func (x *ControlType) SetCalibrationChecking(v bool) {
	x.xxx_hidden_CalibrationChecking = v
}

type ControlType_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	// Indicates if the card was downloaded during the control (bit 'c').
	CardDownloading bool
	// Indicates if the VU was downloaded during the control (bit 'v').
	VuDownloading bool
	// Indicates if a printout was made during the control (bit 'p').
	Printing bool
	// Indicates if the display was used during the control (bit 'd').
	Display bool
	// Indicates if calibration parameters were checked (Gen2+ only) (bit 'e').
	CalibrationChecking bool
}

func (b0 ControlType_builder) Build() *ControlType {
	m0 := &ControlType{}
	b, x := &b0, m0
	_, _ = b, x
	x.xxx_hidden_CardDownloading = b.CardDownloading
	x.xxx_hidden_VuDownloading = b.VuDownloading
	x.xxx_hidden_Printing = b.Printing
	x.xxx_hidden_Display = b.Display
	x.xxx_hidden_CalibrationChecking = b.CalibrationChecking
	return m0
}

var File_wayplatform_connect_tachograph_datadictionary_v1_control_type_proto protoreflect.FileDescriptor

const file_wayplatform_connect_tachograph_datadictionary_v1_control_type_proto_rawDesc = "" +
	"\n" +
	"Cwayplatform/connect/tachograph/datadictionary/v1/control_type.proto\x120wayplatform.connect.tachograph.datadictionary.v1\"\xc8\x01\n" +
	"\vControlType\x12)\n" +
	"\x10card_downloading\x18\x01 \x01(\bR\x0fcardDownloading\x12%\n" +
	"\x0evu_downloading\x18\x02 \x01(\bR\rvuDownloading\x12\x1a\n" +
	"\bprinting\x18\x03 \x01(\bR\bprinting\x12\x18\n" +
	"\adisplay\x18\x04 \x01(\bR\adisplay\x121\n" +
	"\x14calibration_checking\x18\x05 \x01(\bR\x13calibrationCheckingB\xa3\x03\n" +
	"4com.wayplatform.connect.tachograph.datadictionary.v1B\x10ControlTypeProtoP\x01Ztgithub.com/way-platform/tachograph-go/proto/gen/go/wayplatform/connect/tachograph/datadictionary/v1;datadictionaryv1\xa2\x02\x04WCTD\xaa\x020Wayplatform.Connect.Tachograph.Datadictionary.V1\xca\x020Wayplatform\\Connect\\Tachograph\\Datadictionary\\V1\xe2\x02<Wayplatform\\Connect\\Tachograph\\Datadictionary\\V1\\GPBMetadata\xea\x024Wayplatform::Connect::Tachograph::Datadictionary::V1b\x06proto3"

var file_wayplatform_connect_tachograph_datadictionary_v1_control_type_proto_msgTypes = make([]protoimpl.MessageInfo, 1)
var file_wayplatform_connect_tachograph_datadictionary_v1_control_type_proto_goTypes = []any{
	(*ControlType)(nil), // 0: wayplatform.connect.tachograph.datadictionary.v1.ControlType
}
var file_wayplatform_connect_tachograph_datadictionary_v1_control_type_proto_depIdxs = []int32{
	0, // [0:0] is the sub-list for method output_type
	0, // [0:0] is the sub-list for method input_type
	0, // [0:0] is the sub-list for extension type_name
	0, // [0:0] is the sub-list for extension extendee
	0, // [0:0] is the sub-list for field type_name
}

func init() { file_wayplatform_connect_tachograph_datadictionary_v1_control_type_proto_init() }
func file_wayplatform_connect_tachograph_datadictionary_v1_control_type_proto_init() {
	if File_wayplatform_connect_tachograph_datadictionary_v1_control_type_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_wayplatform_connect_tachograph_datadictionary_v1_control_type_proto_rawDesc), len(file_wayplatform_connect_tachograph_datadictionary_v1_control_type_proto_rawDesc)),
			NumEnums:      0,
			NumMessages:   1,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_wayplatform_connect_tachograph_datadictionary_v1_control_type_proto_goTypes,
		DependencyIndexes: file_wayplatform_connect_tachograph_datadictionary_v1_control_type_proto_depIdxs,
		MessageInfos:      file_wayplatform_connect_tachograph_datadictionary_v1_control_type_proto_msgTypes,
	}.Build()
	File_wayplatform_connect_tachograph_datadictionary_v1_control_type_proto = out.File
	file_wayplatform_connect_tachograph_datadictionary_v1_control_type_proto_goTypes = nil
	file_wayplatform_connect_tachograph_datadictionary_v1_control_type_proto_depIdxs = nil
}
